...
namespace {
	...
	uint32_t getLastFrameTime() {
		uint32_t prev_frametimestamp = curr_frametimestamp;
		curr_frametimestamp = SDL_GetTicks();
		return curr_frametimestamp - prev_frametimestamp;
	}
}
	...
	while (!quit_app) {
		...
		while (SDL_PollEvent(&eve)) {
			...
			case SDL_WINDOWEVENT_ENTER:
				preventMouse = false; break;
			case SDL_WINDOWEVENT_LEAVE:
				preventMouse = true; break;			
			case SDL_QUIT:
				quit_app = true;
				break;
			}
		...
		rGUI();
		if(!preventMouse && !io.WantCaptureMouse) {
